package com.example.demo.controller;

import java.util.List;

import org.springframework.http.HttpStatus;
import org.springframework.http.ResponseEntity;
import org.springframework.stereotype.Controller;
import org.springframework.ui.Model;
import org.springframework.web.bind.annotation.GetMapping;
import org.springframework.web.bind.annotation.PostMapping;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.multipart.MultipartFile;

import com.example.demo.handler.fileHandler;
import com.example.demo.model.donations;
import com.example.demo.model.magazine;
import com.example.demo.service.donationsService;
import com.example.demo.service.magazineService;

@Controller
@RequestMapping("/magazine")
public class magazineController {
	
	private magazineService ds;
//	private donationsService serv;
	
	public magazineController(magazineService ds) {
		super();
		this.ds = ds;
//		this.serv = serv;
	}
	
	@GetMapping("/display")
	public String display() {
		return "magazine";
	}
	
	@PostMapping("/insertMagazine")
	public ResponseEntity<magazine> saveMagazine(magazine ob) {
		return new ResponseEntity<magazine>(ds.saveMagazine(ob),HttpStatus.CREATED);
	}
	
	@GetMapping("/showMagazines")
	public String showMagazines(Model m) {
		List<magazine> u=ds.getAllMagazines();
		m.addAttribute("users",u);
		return "magazineDisplay";
	}
	
	@PostMapping("/submit")
	public String uploadFile(magazine u,MultipartFile photo) {
		String path="D:\\Users\\User\\eclipse-workspace\\donations\\src\\main\\webapp\\uploads";
		fileHandler.saveFile(photo, path);
		u.setFile(photo.getOriginalFilename());
		u.setApproved("pending");
//		u.setType("Magazine");
		ds.saveMagazine(u);
//		donations d=new donations(u.getDnid(),u.getType());
//		serv.saveDonation(d);
		return "magazine";
	}

}
